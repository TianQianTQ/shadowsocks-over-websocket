...
TCPRelay.prototype.handleConnectionByServer = function(connection) {
  ...
  connection.on('close', function(code, reason) {
    logger.info(
      `[${connectionId}]: close event[code = '${WSErrorCode[code]}'] of local connection has been triggered`,
    );
...
TCPRelay.prototype.handleConnectionByLocal = function(connection) {
  ...
  connection.on('data', function(data) {
    ...
    switch (stage) {
      // 选择认证方法
      case STAGE_INIT:
        // 第一个数据包 <Buffer 05 01 00>  05 表示  sock 版本号为 5， 01 表示 认证方法长度为1  00 表示 不需要认证
        if (data.length < 3 || data.readUInt8(0) != 5) {
          stage = STAGE_DESTROYED;
          return connection.end();
        }
        // 返回 \x05\x00  0x05 表示 sock 版本号为 5, 0x00 表示 不需要认证
      ...
      case STAGE_ADDR:
        ...
        serverConnection.on('message', function(data) {
          ...
          canWriteToLocalConnection &&
            connection.write(encryptor.decrypt(data), function() {
              logger.debug(
        ...
        serverConnection.on('close', function(code, reason) {
          logger.info(
            `[${connectionId}]: close event[code = '${WSErrorCode[code]}'] of server connection has been triggered`,
          );
          stage = STAGE_DESTROYED;
          connection.end();
        });
